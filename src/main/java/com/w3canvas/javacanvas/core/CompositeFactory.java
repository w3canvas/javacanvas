...
/**
 * Backend-agnostic factory for creating compositing operations according to Canvas 2D API spec.
 * Supports both Porter-Duff composite operations and CSS blend modes.
 *
 * <p>This factory converts Canvas API operation strings (e.g., "source-over", "multiply")
 * into backend-agnostic {@link CompositeOperation} enums, then delegates to the appropriate
...
public class CompositeFactory {
     ...
     *
     * @param operation the composite operation name (e.g., "source-over", "multiply")
     * @param alpha the global alpha value (0.0 to 1.0)
     * @param backend the graphics backend to create the composite for
     * @return a backend-specific IComposite implementation, or null if backend is unknown
     */
    public static IComposite createComposite(String operation, double alpha, IGraphicsBackend backend) {
        ...
        if (backend instanceof AwtGraphicsBackend) {
            return new AwtComposite(compositeOp, alpha);
        } else if (backend instanceof JavaFXGraphicsBackend) {
            return new JavaFXComposite(compositeOp);
        }
        return null;
    }
